...
class Catalog extends \yii\easyii\components\API
{
    ...
    public function api_items($options = [])
    {
        if(!$this->_items){
            ...
            $query = Item::find()->with(['seo', 'category'])->andWhere(['status' => Item::STATUS_ON])->multilingual();

            if(!empty($options['where'])){
                $query->andFilterWhere($options['where']);
            }
            if(!empty($options['not_in'])){
                $query->andFilterWhere(['not in', $options['not_in']['attribute'], $options['not_in']['values']]);
            }
    ...
    public function api_last($limit = 1, $where = null)
    {
        ...
        $query = Item::find()->with('seo')->andWhere(['status' => Item::STATUS_ON])->limit($limit)->orderBy('time DESC');
        if($where){
    ...
    public function api_get($params)
    {
        if(!isset($this->_item[$params[0]])) {
            $this->_item[$params[0]] = $this->findItem($params);
        }
    ...
    private function findItem($params)
    {
        $item = Item::find()->where(['or', 'item_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $params[0]])->andWhere(['status' => Item::STATUS_ON]);

        if(isset($params[1])) {
            $item->localized($params[1]);
        }
